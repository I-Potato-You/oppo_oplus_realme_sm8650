...
# ===== 设置自定义参数 =====
echo "===== 欧加真MT6989通用6.1.134 A16 (天玑特供)OKI内核本地编译脚本 By Coolapk@南城定制内核 ====="
echo ">>> 读取用户配置..."
...
if [[ "$KSU_BRANCH" == "y" || "$KSU_BRANCH" == "Y" ]]; then
  ...
  echo ">>> 最终版本字符串: v${KSU_API_VERSION}-${GIT_COMMIT_HASH}@南城定制内核"
